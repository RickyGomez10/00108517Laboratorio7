...
Matrix createLocalK(int e,mesh &m){
    ...
    //ct,ck,cd,cl,cn,f_x,f_y

    float ct,ck,cd,cl,cn,Ae,J,D;
    ...
    productRealMatrix((ct*J)/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);

    //Preparando matrixK (En clase conocida simplemente como K)
    ck = m.getParameter(CK);
    Ae = calculateLocalArea(e,m);
    transpose(Alpha,Alphat);
    transpose(Beta,Betat);
    productRealMatrix((ck*Ae)/(D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,Beta,2,2,6),2,2,6),6,2,6),matrixK);

    //Preparando matrixG (En clase conocida simplemente como G)
    //CAMBIAR MATRIZ
    cd = m.getParameter(CD);
    calculateBPrima(BPrima);
    productRealMatrix(-(cd*Ae)/(8*(D*D)),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixG);


    //Preparando MatrixH
    cl = m.getParameter(CL);
    productRealMatrix((cl*J)/(9*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixH);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,matrixH,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    b.push_back((cn*J)/6); b.push_back((cn*J)/6); b.push_back((cn*J)/6);
